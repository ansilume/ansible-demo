...
(`firewall_zone`, `firewall_services`, `firewall_default_*`).
...
> `firewall_services` (default `ssh`) before applying.
...
| `firewall_zone` | `public` | Zone to add rules to on RHEL (firewalld only) |
| `firewall_services` | `[ssh]` | firewalld service names to allow (firewalld only) |
